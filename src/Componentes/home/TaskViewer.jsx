    ...
    >
      ...
      <div className="relative ml-auto mr-1.5 mt-1.5">
        <button onClick={toggleMenu}>
          <EllipsisVertical className={isMouseIn ? "opacity-100 cursor-pointer" : "opacity-0"} />
        </button>
        {showSubMenu &&
          <div className="absolute bg-[#0e0e0e] border border-white/25 w-40 right-full rounded">
            <ul>
              <li className="p-1"><button className="w-full px-2 py-1 hover:bg-black/60" onClick={renameColumn}>Rename</button></li>
              <li className="p-1"><button className="w-full px-2 py-1 hover:bg-black/60" onClick={deleteColumn}>Delete</button></li>
            </ul>
          </div>
        }
      </div>
      {
        editColumn ?
          <div className="flex flex-col">
            <input type="text" onChange={changeColumnName} autoFocus className="w-50 m-auto outline-0 border-b border-white/50 px-2 py-1 focus:border-white/80" /> 
