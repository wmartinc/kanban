...
const baseClasses = "absolute text-center md:bottom-15 md:right-0 w-full bottom-2 right-0 h-35"
const variants = {
  alert: "flex items-center",
  confirmation: "flex-col",
  prompt: "animate-bounce flex-col"
}

const AlertDialog = ({ title, description, onConfirm, onCancel, variant = "confirmation" }) => {
  return (
    <div className={`${baseClasses} ${variants[variant]} bg-neutral-950 flex gap-4 sm:w-80 h-auto animacion border border-white/20 p-2 text-white rounded-md`}>
      <div className="flex">
        ...
        <div className="flex items-center flex-col flex-1">
          <h1 className="text-lg font-semibold">Se han hecho cambios</h1>
          <p className="text-sm text-ellipsis max-w-50 overflow-hidden text-nowrap">Desea guardar los cambios?</p>
        </div>
      </div>
